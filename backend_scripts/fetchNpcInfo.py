...
def get_npc_names_retail(timeout: int = 10) -> Dict[str, Dict[int, str]]:
    ...
    session.headers.update({"User-Agent": "Mozilla/5.0 (compatible; npc-names-collector/1.0)"})
...
def save_npc_names_for_db_ids(connection, cursor):
    npc_ids = databaseConnector.fetch_distinct_npc_ids(connection, cursor)  # returns list[int]
    ...
    for locale, id_map in names_by_locale.items():
        # id_map: dict[int, str]
        filtered_map = {str(npc_id): name for npc_id, name in id_map.items() if int(npc_id) in npc_id_set}
        if filtered_map:
